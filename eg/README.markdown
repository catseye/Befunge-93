...
| [digitalr.bf](digitalr.bf) |         | Keymaker            | 9/3/108  | Claims to computes the digital root of the input (like `numer.bf`) but the exact method is uses is unclear. Perhaps this makes it even better suited to numerological divination? Anyway, the input must be terminated with ASCII NUL (character 0; can sometimes be typed in as Ctrl+@.)
